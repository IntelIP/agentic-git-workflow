...
For squash merges, validation commands run against the exact merged commit while checkpoint evidence remains bound to the separately recorded pull-request head. Annotated tags use a deterministic automation tagger, verbatim approved titles, and exact object/message reconciliation. The CLI returns the schema-valid receipt and its local `receiptPath` as separate fields.
